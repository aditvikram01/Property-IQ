...
//
//   openai: OPENAI_API_KEYS | OPENAI_API_KEY | legal_prop | legal_1 | legal
//           model OPENAI_MODEL (default gpt-4o-mini)
//   gemini: GEMINI_API_KEYS | GEMINI_API_KEY | Gemini_API_Key_2 | GEMINI_API_KEY_2 | legal
//           model GEMINI_MODEL (default gemini-2.5-flash)
//   groq:   GROQ_API_KEYS | GROQ_API_KEY | groq_1 | GROQ_1
//           model GROQ_MODEL (default llama-3.3-70b-versatile) — OpenAI-compatible
//
// Preference order defaults to OpenAI → Gemini → Groq; override with the env var
// LLM_PROVIDER_ORDER (comma-separated, e.g. "gemini,groq,openai"). Only providers
// that have a key are used; any omitted from the list are appended, never dropped.
...
const AVAILABLE = {
  openai: OPENAI_CALL ? { name: "openai", model: OPENAI_MODEL, call: OPENAI_CALL } : null,
  gemini: GEMINI_KEYS.length ? { name: "gemini", model: GEMINI_MODEL, call: callGemini } : null,
  groq: GROQ_CALL ? { name: "groq", model: GROQ_MODEL, call: GROQ_CALL } : null,
};

// Preference order: LLM_PROVIDER_ORDER if set, else openai → gemini → groq. Any
// known provider not named in the override is appended so it's never dropped.
const DEFAULT_ORDER = ["openai", "gemini", "groq"];
const ORDER = (() => {
  const named = String(process.env.LLM_PROVIDER_ORDER || "")
    .split(/[,\s]+/).map((s) => s.trim().toLowerCase())
    .filter((n) => DEFAULT_ORDER.includes(n));
  return [...named, ...DEFAULT_ORDER.filter((n) => !named.includes(n))];
})();

// Configured providers, in preference order.
function providerList() {
  return ORDER.map((n) => AVAILABLE[n]).filter(Boolean);
}
