...
// related_chunks, and hand ONLY those chunks to the model (OpenAI when set, else
// Gemini — see lib/llm.js), which writes a warm, source-cited eligibility report +
// action plan. No India Kanoon calls at query time; no secrets needed for
// retrieval. The AI key stays in env.
...
export async function runRagEligibility(form) {
  ...
  const raw = await generate({
    system: SYSTEM,
    prompt: buildPrompt(form, ctxText, today),
    json: true,
    temperature: 0.2,
    schema: ELIG_SCHEMA,
  });
