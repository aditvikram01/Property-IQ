...
//
// Run: node scripts/refresh-db.mjs   (needs an AI key: OPENAI_API_KEYS/legal_1 or
// GEMINI_API_KEYS/legal; INDIAN_KANOON_TOKEN optional)

import { readFile, writeFile } from "node:fs/promises";
import { fileURLToPath } from "node:url";
import { RAG_DB } from "../lib/ragDatabase.js";
import { generate, providerStatus } from "../lib/llm.js";
...
async function extractChange(ruleLabel, sourceText) {
  if (!sourceText || !providerStatus().provider) return null;
...
"""${sourceText.slice(0, 4000)}"""`;
  try {
    const txt = await generate({ prompt, json: true, temperature: 0 });
    const obj = JSON.parse(String(txt).replace(/^```(?:json)?/i, "").replace(/```$/i, "").trim());
...
async function main() {
  ...
  console.log(`[refresh] India Kanoon token: ${IK_TOKEN ? "set" : "MISSING"} | AI provider: ${ai.provider || "NONE"} (${ai.model || "-"})`);
  ...
  for (const rule of RAG_DB.eligibility_rules || []) {
    ...
    const found = await extractChange(label, pieces.join("\n\n"));
    if (found) {
