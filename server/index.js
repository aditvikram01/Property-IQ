...
const server = http.createServer(async (req, res) => {
  ...
  // Server-side "Decode Contract" — same handler as the Vercel api/understand.js
  // function, so the Gemini key never reaches the browser here either.
  if (url.pathname === "/api/understand" && req.method === "POST") {
    try {
      const body = await readBody(req);
      const { op, text, language } = body || {};
      if (op !== "explain" && op !== "risk") return send(res, 400, { error: "op must be 'explain' or 'risk'." });
      if (!text || String(text).trim().length < 30) return send(res, 400, { error: "Add a contract first — paste at least a clause of text." });
      if (!GEMINI_KEYS.length) return send(res, 503, { error: "The AI service is not configured on the server (set GEMINI_API_KEYS or legal)." });
